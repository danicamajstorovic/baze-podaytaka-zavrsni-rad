...
/* Rezervacija */
GO
CREATE PROCEDURE Dodaj_Rezervacija @korisnik_id INT, @frizerskisalon_id INT, @frizer_id INT, @datum NVARCHAR(50)
AS
BEGIN
INSERT INTO Rezervacija(korisnik_id, frizerskisalon_id, frizer_id, datum)
VALUES(@korisnik_id, @frizerskisalon_id, @frizer_id, @datum)
END
GO
DROP PROCEDURE Dodaj_Rezervacija
...
SELECT datum, vreme, Frizer.ime, FrizerskiSalon.adresa FROM Rezervacija JOIN Frizer ON Frizer.id = frizer_id JOIN FrizerskiSalon ON FrizerskiSalon.id = frizerskisalon_id  WHERE  korisnik_id = 

SELECT adresa From FrizerskiSalon
SELECT ime,prezime FROM Frizer